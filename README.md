...
- ### 执行脚本
    ...
    - #### 并发执行：
        ...
        > 含义：为每个账号单独在后台启动一个进程运行脚本，由于是后台运行故不在命令行输出进度直接记录到日志中\
...
- ### 1. 终止运行中的脚本：
    ...
    > 注：根据脚本名称搜索对应的进程并立即杀死，当脚本报错死循环时可使用此功能。
...
- ### 6. 账号功能：
    ...
    - #### 固定可选参数：
        ...
        > `update`ㅤ使用 `WSKEY` 更新CK ，需要在 account.json 中正确配置您的信息，注意格式，ep为设备信息，可不填会随机项目一些内置的参数\
