...
let GO_CQHTTP_MSG_SIZE = '1500' // 外网扫码地址
let GO_CQHTTP_EXPIRE_SEND_PRIVATE = true
let qqnumber = ''
let accounts = []
...
function nameConvert(pt_pin, remarks = '', text) {
    ...
    try {
        pt_pin = decodeURI(pt_pin);
    } catch (e) {
    }
    if (typeof remarks === 'string') {
        return text.replace(new RegExp(pt_pin, 'gm'), remarks)
    } else if (typeof remarks === 'object' && Array.isArray(remarks) && remarks.length > 0) {
        return text.replace(new RegExp(pt_pin, 'gm'), remarks[0])
    } else {
        return text;
    }
}
...
function goCQhttp(text = '', desp = '') {
    if (GO_CQHTTP_URL && GO_CQHTTP_QQ && GO_CQHTTP_METHOD) {
        ...
        if (accounts) {
            for (let account of accounts) {
                let remarks = account['remarks'] || [];
                if ((typeof remarks === 'string' && desp.indexOf(account['remarks']) > -1) || (Array.isArray(remarks) && remarks.length > 0 && remarks.filter(n => text.indexOf(n) > -1 || desp.indexOf(n) > -1).length > 0)) {
                    qqnumber = account['qqnumber']
                }
            }
        }
        let expire = RegExp("重新登录");
        let individual = RegExp("【私信】|已可领取|已可兑换|已成熟|未选择|兑换成功|提现成功|忘了|京东资产变动|东东农场|未继续|已领取|兑换已超时");
        if ((expire.test(text) === true || expire.test(desp) === true) && GO_CQHTTP_EXPIRE_SEND_PRIVATE && qqnumber) {
            let msgs = (msg + '\n\n' + GO_CQHTTP_LINK)
            return new Promise(resolve => {
                $.get({
            ...
            let pageSize = parseInt(GO_CQHTTP_MSG_SIZE);
            if (desp.length > pageSize && GO_CQHTTP_METHOD === "send_group_msg") {
                return Promise.all(textPage(desp, pageSize).map((item) => {
                    return goCQhttp(text, item);
                }))
            }
            return new Promise(resolve => {
                $.get({
...
function CoolPush(text, desp) {
    return new Promise(resolve => {
        if (QQ_SKEY) {
            ...
            console.log('您未提供QQ酷推所需的 SKEY，取消QQ酷推推送消息通知🚫');
            resolve()
        }
    })
}
