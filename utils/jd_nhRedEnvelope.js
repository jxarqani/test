...
添加环境变量: JX_CODE 如需自己返利，请填写该变量（https://u.jd.com/后面的英文）
*/
const $ = new Env("京享红包");
...
async function main() {
  ...
  $.max = false;
  $.hotFlag = false;
  const flCodeArr = ['SdIdGkO', 'SCdH83Z', 'SMIdOOm'];
  let flCode = flCodeArr[Math.floor(Math.random() * flCodeArr.length)];
  let FLCODE = $.isNode() ? process.env.JX_CODE ? process.env.JX_CODE : '' : $.getdata("JX_CODE") ? $.getdata("JX_CODE") : ''
  if (FLCODE) {
    if ($.index === 1) {
      flCode = FLCODE;
    } else {
