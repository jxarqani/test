...
返利变量：JX_CODE，若需要返利给自己，请自己修改环境变量[JX_CODE]换成自己的返利
export JX_CODE=""
...
export JX_REBATE_PIN="pin1,pin2"

分享码
export JX_SHARE_CODE=""
*/
let rebateCodes = ''
let rebatePin = ''
const $ = new Env('年货节红包');
...
rebatePin = $.isNode() ? (process.env.JX_REBATE_PIN ? process.env.JX_REBATE_PIN : `${rebatePin}`) : ($.getdata('JX_REBATE_PIN') ? $.getdata('JX_REBATE_PIN') : `${rebatePin}`);
...
!(async () => {
  ...
  if (nowTime > new Date(activeEndTime).getTime()) {
    ...
    $.setdata('','JX_SHARE_CODE')
    return
  }
  $.shareCodeArr = $.getdata('JX_SHARE_CODE') || {};
  // $.shareCodeArr = {};
...
async function run(type = 0){
  try{
    rebateCodes = ['SdIdGkO', 'PtDU81O', 'PtDU81O', 'SMIdOOm'];
